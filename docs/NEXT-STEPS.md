...
| Model comparison | Qwen3-4B Q5, Q4, Llama-3.2-3B Q4 and scripted baseline have versioned comparison reports with checksum, latency, VRAM/RAM, validity and guardrail results. Prompt v4 removed Q5 timeouts and guardrail rejections on the retained corpus, but only landed correctly in 7/10 terminal situations. | Grow the held-out mission corpus; do not select a winner from the current inadequate model results. |
