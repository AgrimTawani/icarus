...
- [x] Narrow route. The route is intentionally narrower than the fixed 2.5 m
      planner envelope. The Phase 12 acceptance asserts safe
      `ABORTED_BY_SAFETY` refusal with `no collision-free path` rather than
      lowering the envelope to force a traversal.
